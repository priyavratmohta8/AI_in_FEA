...
=================================================================================
Uses the surrogate model saved by step3_finalize_and_save.py (random forest)
or train_gaussian_process.py (GP + scaler bundle) to search for the
combination of (length, width, thickness, height) that minimizes mass while
keeping predicted FOS above a target value.
...
# -----------------------------------------------------------------------
MODEL_PATH = "Python Codes/fos_surrogate_gp.pkl"
...
#   - Random forest (step3): a bare model, no scaler needed.
#   - GP (train_gaussian_process.py): a {"model": ..., "scaler": ...} dict,
#     since the GP MUST see inputs scaled the same way it was trained on.
loaded = joblib.load(MODEL_PATH)

if isinstance(loaded, dict):
    model = loaded["model"]
    scaler = loaded["scaler"]
    print(f"Loaded GP surrogate (with scaler) from {MODEL_PATH}")
else:
    model = loaded
    scaler = None
    print(f"Loaded random forest surrogate from {MODEL_PATH}")
...
def compute_mass(pop):
    ...
    NOTE: always called with RAW (unscaled) values -- mass is an exact
    physical formula, not a model prediction, so scaling would corrupt it.
    """
    length    = pop[:, 0]
    width     = pop[:, 1]
    thickness = pop[:, 2]
...
def fitness(pop):
    """
    pop: array of shape (N, 4), RAW (unscaled) values
    returns: array of shape (N,) -- lower is better
    """
    pop_for_model = scaler.transform(pop) if scaler is not None else pop
    fos_pred = model.predict(pop_for_model)   # surrogate's guess at FOS for each design
    mass = compute_mass(pop)                  # exact, computed directly, always raw
...
if __name__ == "__main__":
    ...
    best_design_for_model = scaler.transform(best_design_2d) if scaler is not None else best_design_2d
    predicted_mass = compute_mass(best_design_2d)[0]  # always raw
    ...
    if scaler is not None:
        # GP: report predicted uncertainty alongside the prediction --
        # this is the signal that would have flagged the original
        # failing corner (predicted 1.5, actual 0.83) as untrustworthy.
        pred_fos, pred_std = model.predict(best_design_for_model, return_std=True)
        pred_fos = pred_fos[0]
        pred_std = pred_std[0]
        print(f"\n  Predicted FOS:  {pred_fos:.4f} ± {pred_std:.4f}  (target: {FOS_TARGET})")
        if pred_std > 0.15 * abs(pred_fos):
            print(f"  ⚠️  High uncertainty relative to the prediction -- treat this")
            print(f"      design as UNVERIFIED. Run it through ANSYS before trusting it,")
            print(f"      and consider adding it to your training data regardless of outcome.")
    else:
        pred_fos = model.predict(best_design_2d)[0]
        print(f"\n  Predicted FOS:  {pred_fos:.4f}  (target: {FOS_TARGET})")
